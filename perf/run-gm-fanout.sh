#!/bin/bash
#
# Run GM messaging fanout performance test:
#
...
# - setting subscriberConnections will run LB subscribers
...
do
	$SDK_PERF/sdkperf_java.sh -cip="$SMF_URL" -cu=$MSG_USER@$VPN -cp=$MSG_PASS -stp=$topic -stc=$topicCount -sql=$queue$var -pe -pea 0 -cc $subscriberConnections >sub$var.out 2>sub$var.err &
	subPids+=($!)
done
...
$SDK_PERF/sdkperf_java.sh -cip="$SMF_URL" -cu=$MSG_USER@$VPN -cp=$MSG_PASS -ptp=$topic -ptc=$topicCount -mt=persistent -msa=$messageSize -mn=$messageCount -mr=$publishRate -cc $publisherConnections>pub.out 2>pub.err &
pubPid=$!
